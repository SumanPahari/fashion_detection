...
from backbone.res_18 import ResNet18
...
class Bottleneck18(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, stride=1, downsample=None, norm_layer=None):
        super(Bottleneck18, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d

        self.conv1 = nn.Conv2d(in_channels=in_channel, out_channels=out_channel,
                               kernel_size=3, stride=1, bias=False,padding=1)  # squeeze channels
...
def resnet18_fpn_backbone():
    resnet_backbone = ResNet(Bottleneck18, [2, 2, 2, 2],include_top=False)
    #resnet_backbone = ResNet18(2)
    ...
    print("in_channels_stage2", in_channels_stage2)
    # in_channels_list = [
