...
from backbone.dcn import DeformableConv2d
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, stride=1, downsample=None, norm_layer=None):
        ...
        self.conv1 = nn.Conv2d(in_channels=in_channel, out_channels=out_channel,
                               kernel_size=1, stride=1, bias=False,dilation=1)  # squeeze channels
        self.bn1 = norm_layer(out_channel)
        # -----------------------------------------
        self.conv2 = nn.Conv2d(in_channels=out_channel, out_channels=out_channel,
                               kernel_size=3, stride=stride, bias=False, padding=1,dilation=1)
        self.bn2 = norm_layer(out_channel)
        # -----------------------------------------
        self.conv3 = nn.Conv2d(in_channels=out_channel, out_channels=out_channel * self.expansion,
                               kernel_size=1, stride=1, bias=False,dilation=1)  # unsqueeze channels
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, blocks_num, num_classes=1000, include_top=True, norm_layer=None):
        ...
        self.conv1 = DeformableConv2d(3, self.in_channel, kernel_size=7, stride=2,
    ...
    def _make_layer(self, block, channel, block_num, stride=1):
        ...
        if stride != 1 or self.in_channel != channel * block.expansion:
            downsample = nn.Sequential(
                nn.Conv2d(self.in_channel, channel * block.expansion, kernel_size=1, stride=stride, bias=False,dilation=1),
...
class FeaturePyramidNetwork(nn.Module):
    ...
    def __init__(self, in_channels_list, out_channels, extra_blocks=None):
        ...
        for in_channels in in_channels_list:
            if in_channels == 0:
                continue
            inner_block_module = DeformableConv2d(in_channels, out_channels, 1)
            layer_block_module = DeformableConv2d(out_channels, out_channels, 3, padding=1)
